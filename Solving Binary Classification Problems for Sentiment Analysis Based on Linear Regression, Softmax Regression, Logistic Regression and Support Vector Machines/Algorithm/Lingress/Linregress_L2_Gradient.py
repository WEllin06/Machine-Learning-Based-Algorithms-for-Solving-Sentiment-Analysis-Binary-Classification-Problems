...
# 计算岭回归的梯度
def ridge_regression_gradient(X, y, w, alpha):
    n_samples = X.shape[0]
    error = y - np.dot(X, w)
    gradient = -2 * np.dot(X.T, error) + 2 * alpha * w
    return gradient

# 使用随机梯度下降求解参数 w
def ridge_regression_sgd(X, y, alpha=1.0, learning_rate=0.01, num_iterations=1000, tolerance=1e-4):
    # 添加偏置项
    ...
    for i in range(num_iterations):
        ...
        for idx in indices:
            xi = X_with_bias[idx]
            yi = y[idx]
            gradient = ridge_regression_gradient(xi.reshape(1, -1), yi, w, alpha)

            # 更新权重参数
            w -= learning_rate * gradient

        # 计算损失
        loss = ridge_regression_loss(X_with_bias, y, w, alpha)
...
# 使用随机梯度下降求解参数 w
alpha = 0.0000001  # 正则化参数
learning_rate = 0.00001  # 学习率
num_iterations = 1000  # 迭代次数
tolerance = 1e-4  # 容忍度
weights = ridge_regression_sgd(X_train, y_train, alpha=alpha, learning_rate=learning_rate,
